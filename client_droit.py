...
la balle en fonction des données envoyees par le serveur et reset replace la balle au centre. On teste constamment la 
longueur de la chaine de caractère donneesrecues pour s'assurer que le joueur adverse est toujours connecté et ne pas 
causer d'erreur.'''
...
Cette fonction s'occupe du déplacement de la raquette de gauche avec les touches UP et DOWN.
Le mouvement de la raquette de droite est assuré par le premier paramètre envoye par le serveur.
Elle est appelé indéfiniment dès que le joueur lance le jeu. On teste constamment la longueur de la chaine de caractère
donneesrecues pour s'assurer que le joueur adverse est toujours connecté et ne pas causer d'erreur.'''


def mouvement_raquette(keys, raquette_gauche, raquette_droite):
    if len(donneesrecues) == 9:
        raquette_gauche.y = int(donneesrecues[0])

    if keys[pygame.K_UP] and raquette_droite.y - raquette_droite.VITESSE_RAQUETTE >= 0:
        raquette_droite.mouvement(up=True)
    if keys[
        pygame.K_DOWN] and raquette_droite.y + raquette_droite.VITESSE_RAQUETTE + raquette_droite.hauteur <= HAUTEUR:
        raquette_droite.mouvement(up=False)
...
def play_function(taille: List, font: 'pygame.font.Font', vitesse: List, winning_score_selected: List,
    ...
    while True:
        ...
        client.send(
            bytes(
                str(f"{raquette_gauche.y},{raquette_droite.y},{taille_raquette},{vitesse_balle},{winning_score}").encode(
                    'utf-8')))
...
def main(test: bool = False) -> None:
    ...
    parametre_menu.add.selector('Couleur raquettes et balle ',
                                [('Blanc', 'BLANC'), ('Bleu', 'BLEU'), ('Vert', 'VERT'), ('Rouge', 'ROUGE'),
                                 ('Noir', 'NOIR'), ('Rose', 'ROSE'), ('Orange', 'ORANGE'), ('Jaune', 'JAUNE')],
    ...
    parametre_menu.add.selector("Couleur fond d'écran ",
                                [('Noir', 'NOIR'), ('Blanc', 'BLANC'), ('Vert', 'VERT'), ('Bleu', 'BLEU'),
                                 ('Rouge', 'ROUGE'), ('Rose', 'ROSE'), ('Orange', 'ORANGE'), ('Jaune', 'JAUNE')],
    ...
    parametre_menu.add.selector("Couleur score et texte ",
                                [('Blanc', 'BLANC'), ('Noir', 'NOIR'), ('Vert', 'VERT'), ('Bleu', 'BLEU'),
                                 ('Rouge', 'ROUGE'), ('Rose', 'ROSE'), ('Orange', 'ORANGE'), ('Jaune', 'JAUNE')],
    ...
    main_menu.add.button('Lancer la partie', play_function,
                         TAILLE,
                         pygame.font.Font(pygame_menu.font.FONT_FRANCHISE, 30), VITESSE, WINNING_SCORE)
